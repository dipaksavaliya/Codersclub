...
//    $profile=$_SESSION['uid'];
    ...
    $qrypf="SELECT login.fname, login.lname, login.gender, login.email, login.skill, country.cname, state.sname, city.city, login.addskill, login.companyname, login.companyadd, login.userid FROM login inner join state on login.state=state.sid 
inner join country on login.country=country.cid
inner join city on login.city=city.cityid 
inner join user on login.userid=user.userid where user.userid='$profile' ";
